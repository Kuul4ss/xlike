  ...
  return RepositoryProvider(
    ...
    child: MultiBlocProvider(
      ...
      child: const MaterialApp(
        home: UserPostsScreen(userId: 30),
      ),
    ),
  );
}
...
void main() {
  group('$UserPostsScreen', () {
    testWidgets('$UserPostsScreen should display the right title', (WidgetTester tester) async {
      await tester.pumpWidget(_setUpPostsScreen(PostsApiDataSource()));
      await tester.pump(const Duration(seconds: 2));
      expect(find.text('Posts d\'un utilisateur'), findsOneWidget);
    });

    testWidgets('$UserPostsScreen should display an error if an error occurred', (WidgetTester tester) async {
      await tester.pumpWidget(_setUpPostsScreen(ErrorDataSource()));
      await tester.pump(const Duration(seconds: 2));
      expect(find.text('Oups, une erreur est survenue.'), findsOneWidget);
    });

    testWidgets('$UserPostsScreen should display a loading indicator', (WidgetTester tester) async {
    ...
    testWidgets('$UserPostsScreen should display a loader when Loading', (WidgetTester tester) async {
      await tester.pumpWidget(_setUpPostsScreen(PostsApiDataSource()));
      expect(find.byType(CircularProgressIndicator), findsOneWidget);
